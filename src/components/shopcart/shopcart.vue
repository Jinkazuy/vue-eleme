<template>
  <div id="shopcart">
    ...
    <div class="cartinfo">
      ...
      <div class="carticonoutline" @click="showThisCart" ref="thisCart">
        <div :class="totalNumber !== 0 ? 'havebg carticon' : 'carticon'">
          ...
          <i :class="totalNumber !== 0 ? 'haveicon iconfont' : 'iconfont'"
            >&#xe609;</i
          >
        </div>
      </div>
      <!--商品价格-->
      <span :class="totalPrice !== 0 ? 'pricetrue price' : 'price'"
        >￥{{ totalPrice }}</span
      >
      <!--竖线-->
    ...
    <!--右侧结算按钮，固定宽度-->
    <!--这里不做支付详情页了，值做一个弹框示意一下-->
    <div :class="[payButtonClass]" @click="pay">{{ priceDifference }}</div>
    <!--购物车浮层-->
      ...
      <div class="sellercart" v-show="cartShow">
        ...
        <div class="carcont">
          ...
          <div class="listScroll">
            ...
            <div>
              <div
                class="cartGoodsLest border-1px"
                v-show="sellerCartList.goods.length"
                v-for="item in sellerCartList.goods"
              >
                <span class="goodsName">{{ item.goodsInfo.name }}</span>
                <span class="goodsPrice"
                  >￥<span class="num">{{ item.goodsInfo.price }}</span></span
                >
                <!--这里又用到了加减号组件-->
                ...
                <addgoods
                  class="goodsAdd"
                  :thisgoodsinfo="item.goodsInfo"
                  :sellername="sellerCartList.seller"
                  @getAddDom="getAddDomByCart"
    ...
    <!--背景模糊层-->
    <!--其实可以直接设置宽高为100%，因为用了fixed布局，就已经脱标了-->
    <!--但这里使用js操作DOM的宽高 ，其实就是为了练习window.innerWidth和window.innerHeight-->
...
export default {
  ...
  data() {
    return {
      ...
      sellerCartList: {},
      // 用于监听,接收数据，然后缓存起来的 sellerCartList 购物车中是否有商品；
      sellerCartListGoods: {},
      // 商品总数
      totalNumber: 0,
      // 商品总价价格
  ...
  methods: {
    ...
    showThisCart() {
      ...
      // 点击购物车按钮的时候判断，该商家下的购物车中是否有商品，如果没有就不展开，如果有商品就展开；
      if (this.sellerCartListGoods.length) {
        // console.log('没有');
        // 既然没有商品了，那么就关闭购物车列表；
        this.cartShow = true;
      }
      // 而且还要用xxxxxx监听 this.sellerCartList.goods ，如果为0的话，也让购物车列表的状态为false；
      // 比如：在购物车列表点击-号的时候，把最后一个商品减完，什么都没有了，此时就应该关闭列表；
    },
    // 小球动画
    showBall(addDOM) {
    ...
    beforeEnter(el) {
      ...
      el.style.transform = `translate(0px, ${this.addButton.getBoundingClientRect()
        .y - 10}px)`;
      el.children[0].style.transform = `translate(${this.addButton.getBoundingClientRect()
        .x - 35}px, 0px)`;
    ...
    enter(el, done) {
      ...
      el.style.transform = `translate(10px, ${
        this.$refs.thisCart.getBoundingClientRect().y
      }px)`;
      el.children[0].style.transform = `translate(-${
        this.$refs.thisCart.getBoundingClientRect().x
      }px, 0px)`;
    ...
    afterEnter(el) {
      ...
      // 让小球隐藏；
      // TODO:其实这里还可以做一个小球动画执行完，再让购物车角标变鼓起来的一个效果(通过控制DOM元素的方法比较方便，如果不会做就用animated的动画插件)；
    ...
    pay() {
      ...
      alert('去结算啦');
      // 真是开发中，肯定不能用这个组件缓存的购物车商品直接提交过去，因为可能会有客户端篡改；
      // 那么比较保险就是，将商家名称传到结算页面，然后结算页拿到商家名称，
      // 再做ajax请求，找到该用户数据的购物车中与商家名称相对应的数据，然后在这个数据中将所有商品遍历出来；
      // 不过此时客户端想要改还是可以的，但后台会验证数据；
    }
  },
  watch: {
    // 这里监听购物车中是否有商品存在，如果没有，将让购物车列表关闭，
    // 应对情况就是，当用户在购物车列表中操作商品数量，只到把最后一个商品减没。
    // 那么购物车中没有数据，就应该关闭；
    sellerCartListGoods: function() {
      if (!this.sellerCartListGoods.length) {
        // console.log('没有');
        // 既然没有商品了，那么就关闭购物车列表；
        this.cartShow = false;
  ...
  computed: {
    getThisSellerCart: function() {
      ...
      for (var i = 0; i < this.$store.getters.getAllcart.length; i++) {
        if (this.$store.getters.getAllcart[i].seller === this.sellerInfo.name) {
          ...
          // 这里没有实际的作用，只是用于watch中监听购物车中的商品是否存在；
          this.sellerCartListGoods = this.sellerCartList.goods;

          // 计算商家购物车中，所有商品的个数
          for (
            var k = 0;
            k < this.$store.getters.getAllcart[i].goods.length;
            k++
          ) {
            this.totalNumber += this.$store.getters.getAllcart[i].goods[k].num;
            this.totalPrice +=
              this.$store.getters.getAllcart[i].goods[k].num *
              this.$store.getters.getAllcart[i].goods[k].goodsInfo.price;
    ...
    priceDifference: function() {
      ...
      if (this.totalPrice >= this.sellerInfo.minPrice) {
        ...
        this.totalPrice > 0 &&
        this.totalPrice < this.sellerInfo.minPrice
      ) {
...
#shopcart
    ...
    .sellercart
        width: 100%;
        // 限制最大高度，这样实现了高度有商品撑开，最大是258px；
        // 同时也要给.listScroll设置最大高度是258px；
        max-height: 264px;
        background-color: #fff;
        position: absolute;
